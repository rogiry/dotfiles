...
rogiry/dotfiles | ⎇ main | 5419984 ✓ | +0-0 | S:0 M:0 ?:0 | - | cwd: ~/.local/share/chezmoi
...
| 2 | 저장소 · 브랜치 · SHA+동기화 · 변경량 · 파일 상태 · PR · CI / **cwd 는 항상 마지막** |
...
| 모드 | 출력 | 비고 |
|---|---|---|
| `branch` | `⎇ main` / `⑂ feature` | `⑂` 는 연결된 워크트리. 글리프만 바뀌어 자리를 더 안 먹는다 |
| `commit` (=`sync`) | `5419984 ✓` / `5419984 ↑2↓3` / `5419984 ⚠` | ✓ 동기화됨, ↑푸시 필요, ↓풀 필요, ⚠ upstream 미설정 |
...
| 함정 | 결과 |
|---|---|
| `custom-command` 출력은 **`.trim()` 된다** | 앞뒤 공백으로 간격을 못 만든다. 그래서 `commit` 모드가 SHA 와 ✓ 를 **함께** 출력한다 |
